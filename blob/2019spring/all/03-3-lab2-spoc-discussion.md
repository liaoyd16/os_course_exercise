...
3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
   ...
   在中断响应、函数调用或指令执行时，CPL通过对应的门（中断门、陷阱门、调用门和任务门等）访问一个GDT中的描述符；

   DPL是被访问的描述符允许的访问权限级，而RPL是访问者选择的访问优先级，满足CPL<=RPL。而访问成功的条件是RPL<=DPL。
...
5. TSS和Task Register的作用是什么？
   ...
   > The TR register is a 16-bit register which holds a segment selector for the TSS. It may be loaded through the [LTR](https://en.wikipedia.org/wiki/Load_task_register) instruction. [LTR](https://en.wikipedia.org/wiki/Load_task_register) is a privileged instruction and acts in a manner similar to other segment register loads. The task register has two parts: a portion visible and accessible by the programmer and an invisible one that is automatically loaded from the TSS descriptor.

   TR是内容为一个段选择器的寄存器，可以通过LTR指令加载。
...
1. 一条指令执行时最多会出现多少次地址转换？
   ...
   两次：若启动页机制，则首先进行逻辑地址到线性地址转换，再进行线性地址到物理地址转换

2. 描述X86-32的MMU地址转换过程；

   通过段机制：由逻辑地址加上段offset，得到物理地址

   通过页机制：有逻辑地址通过TLB/页表，得到物理地址
